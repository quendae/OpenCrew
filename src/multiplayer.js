...
export class Multiplayer {
  constructor({onState,onLobby,onError}={}) {
    this.role=null; this.room=''; this.localSeat=0; this.peers=new Map(); this.names=[]; this.botSeats={}; this.revision=0;
  ...
  async createRoom(name='Host') {
    const r=await this.request('/room/create',{name}); this.role='host'; this.room=r.code; this.localSeat=0; this.names[0]=name; this.startPolling(); this.emitLobby(); return r.code;
  }
  async joinRoom(code,name='Guest') {
    this.role='guest'; this.room=code.toUpperCase();
  ...
  makePeer(key) {
    ...
    pc.onicecandidate=e=>{if(e.candidate)this.request(`/room/${this.room}/signal`,{from:this.role,type:'ice',to:key,candidate:e.candidate}).catch(this.onError)};
    pc.ondatachannel=e=>this.bindChannel(key,e.channel);
    return pc;
  }
  bindChannel(key,dc){const p=this.peers.get(key)||{pc:null};p.dc=dc;this.peers.set(key,p);dc.onmessage=e=>this.handle(key,JSON.parse(e.data));dc.onclose=()=>this.emitLobby();}
  ...
  startPolling(identity=this.role){clearInterval(this.pollTimer);let cursor=0;this.pollTimer=setInterval(async()=>{try{const r=await this.request(`/room/${this.room}/signals?after=${cursor}`);cursor=r.cursor||cursor;for(const m of r.messages||[])await this.handleSignal(m,identity);}catch(e){this.onError(e)}},900)}
  async handleSignal(m,identity){
    if(this.role==='host'&&m.type==='offer'){
      const key=m.from||crypto.randomUUID(); if(this.peers.has(key))return; const pc=this.makePeer(key); const peer=this.peers.get(key); peer.seat=this.nextSeat();
  ...
  emitLobby(){this.onLobby({role:this.role,room:this.room,names:this.names,botSeats:this.botSeats,localSeat:this.localSeat});}
  setBot(seat,difficulty='normal'){if(this.role!=='host')return;this.botSeats[seat]=difficulty;this.emitLobby();this.broadcast({type:'lobby',names:this.names,botSeats:this.botSeats});}
  ...
  action(action,payload={}){if(this.role==='host')return this.execute?.(this.localSeat,action,payload);this.send('host',{type:'action',action,payload});}
  handle(key,msg){
